...
public class PrincipalScript : MonoBehaviour
{
	...
	private void FixedUpdate()
	{
		if (!angry)
		{
			...
			if (Physics.Raycast(base.transform.position, direction, out hitInfo, float.PositiveInfinity, 3) & (hitInfo.transform.name == "Its a Bully") & (bullyScript.guilt > 0f) & !inOffice & !angry)
			{
				TargetBully();
			}
		}
